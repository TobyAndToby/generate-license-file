...
var License = class {
	...
	format(lineEnding) {
		let formattedText = this.prefix(lineEnding) + this.formatDependencies(lineEnding) + this.midfix(lineEnding) + prepareContentForOutput(this.content.trim(), lineEnding);
		if (this.notices.length > 0) {
			const noticeContent = this.notices.map((notice) => notice.trim()).join(lineEnding);
			formattedText += this.noticesPrefix(lineEnding) + prepareContentForOutput(noticeContent, lineEnding);
		}
