const require_file_utils = require('../../utils/file.utils.cjs');
const require_spinner = require('../spinner.cjs');
const require_argument = require('./argument.cjs');

//#region src/lib/cli/args/inputs.ts
var Inputs = class extends require_argument.Argument {
	question = "Package.json location: ";
	async resolve(config) {
		const { inputs } = config;
		if (!inputs) return await this.resolveOne(void 0);
		if (inputs.length === 1) return await this.resolveOne(inputs[0]);
		return await this.resolveMany(inputs);
	}
	async parse(config) {
		const { inputs } = config;
		if (!inputs || inputs.length === 0) throw new Error("No --input argument given.");
		let allValid = true;
		for (const input of inputs) if (!await require_file_utils.doesFileExist(input)) {
			require_spinner.spinner.warn(`${input} could not be found.`);
			allValid = false;
		}
		if (!allValid && inputs.length === 1) throw new Error("Given --input file not found");
		if (!allValid) throw new Error("One or more given --input files not found");
		return inputs;
	}
	async resolveOne(input) {
		const initialValue = await this.getInputPromptInitialValue();
		let inputExists = input ? await require_file_utils.doesFileExist(input) : false;
		while (!input || !inputExists) {
			if (!!input && !inputExists) require_spinner.spinner.fail("Given --input file not found");
			input = await this.promptForString(this.question, initialValue);
			inputExists = await require_file_utils.doesFileExist(input);
		}
		return [input];
	}
	async resolveMany(inputs) {
		const validInputs = [];
		let allValid = true;
		for (const input of inputs) {
			if (!await require_file_utils.doesFileExist(input)) {
				require_spinner.spinner.warn(`${input} could not be found.`);
				allValid = false;
				continue;
			}
			validInputs.push(input);
		}
		if (!allValid) await this.promptForTermination();
		return validInputs;
	}
	async getInputPromptInitialValue() {
		return await require_file_utils.doesFileExist("./package.json") ? "./package.json" : "";
	}
	async promptForTermination() {
		if (!await this.promptForBoolean("One or more given --input files not found. Do you want to continue?")) throw new Error("Process terminated by user");
	}
};

//#endregion
exports.Inputs = Inputs;