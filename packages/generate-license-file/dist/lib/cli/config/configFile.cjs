...
const require_json5 = require('./parsers/json5.cjs');
let cosmiconfig = require("cosmiconfig");

//#region src/lib/cli/config/configFile.ts
const json5Loader = (_filepath, content) => require_json5.json5Parse(content);
const loadConfig = async (filePath) => {
	const result = await (0, cosmiconfig.cosmiconfig)("generate-license-file", { loaders: {
		".json": json5Loader,
		".jsonc": json5Loader,
		".json5": json5Loader
	} }).load(filePath);
...
const findConfig = async (directory) => {
	const result = await (0, cosmiconfig.cosmiconfig)("generate-license-file", {
		stopDir: directory,
		loaders: {
			".json": json5Loader,
			".jsonc": json5Loader,
