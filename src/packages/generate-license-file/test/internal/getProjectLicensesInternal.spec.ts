...
import console from "../../src/lib/utils/console.utils";
import { doesFileExist, readFile } from "../../src/lib/utils/file.utils";
import { getProject, Project } from "../../src/lib/utils/license.utils";
...
jest.mock("../../src/lib/utils/license.utils", () => ({
  getProject: jest.fn(),
}));

jest.mock("../../src/lib/utils/packageJson.utils", () => ({
  readPackageJson: jest.fn(),
}));

describe("getLicensesForProjects", () => {
  ...
  it("should get the project from license-checker", async () => {
    await getLicensesForProjects([packageJsonPath]);
  ...
  it("should return all license contents for returned dependencies", async () => {
    const result = await getLicensesForProjects([packageJsonPath]);
  ...
  it("should throw if there's no name key in the given package.json", async () => {
    ...
    await expect(getLicensesForProjects([packageJsonPath])).rejects.toThrow(
