...
If a file already exists at the provided output path, then the CLL won't overwrite it by default.
...
When using the the CLI locally, it will prompt you for inputs if you forget to supply one that's mandatory.

To stop the CLI from trying to show the prompts in a CI/CD environment, you can provide the `--ci` flag.
