...
Like the CLI, this method requires an input directory containing the package.json, an output file path,
and can be given an optional line ending paramter. It will produce the same result as running the command
via the CLI.
...
glf
  .generateLicenseFile("./", "./third-party-licenses.txt")
  .then(() => {})
  .catch(error => {});
```
...
to disk, you can use the `getLicenseFileText` method. It accepts the directory of the project's package.json,
and an optional line ending enum.
...
const licenseFileText: string = await getLicenseFileText("./");
...
// Get an array of licenses for the current project's production dependencies.
const licenses: ILicense[] = await getProjectLicenses("./");
```
