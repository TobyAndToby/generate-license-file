...
The `generate-license-file` CLI will resolve all of the licenses of your third-party dependencies and write them to a file on disk.

Below outlines all of the CLI flags that you can give to change generate-license-file's behavior.

For more complex use cases, see the [config file](./config-file.mdx) documentation for additional configuration options that can't
...
The file itself must be named `package.json`. The path tells generate-license-file which project to report on, but the
dependency tree is resolved by npm and pnpm, both of which look for `package.json` by name in each package directory. A
file with any other name can't be used as an input, and will be rejected rather than silently producing an empty output
file.
...
Using `--ci` will implicitly set `--no-spinner`.
...
By default, the CLI includes version numbers in the package names in the generated output file (e.g., "package@1.2.3").
