...
Every method below takes a path to a project's `package.json`. The file must be named `package.json` — the path
identifies which project to report on, but the dependency tree is resolved by npm and pnpm, both of which look for
`package.json` by name. Passing a path to a differently named file will throw.
