...
module.exports = {
  plugins: [
    new LicenseFilePlugin({
      ...
      pathToPackageJson: "./package.json", // Must be a file named package.json.
...
`pathToPackageJson` must point at a file named `package.json`. The path selects which project to report on, but
npm and pnpm both resolve the dependency tree by that file name, so a differently named file cannot be used.

See the docs for the [CLI config file](cli/config-file) for information on how to use the `append`, `replace`, and `exclude` options.
